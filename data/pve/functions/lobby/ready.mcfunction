...
execute unless score Ready InLobby matches 0 if score Ready InLobby = InLobby InLobby run function pve:main/start_check
...
execute unless score Editmode State matches 1 unless score Game State matches 1 if score InLobby InLobby < Ready InLobby run tellraw @a[tag=Operator] [{"text":"More than 60% of the players in the lobby are ready. \n"},{"text":"Start the game?","hoverEvent":{"action":"show_text","contents":[{"text":"Click to Start","italic":true}]},"clickEvent":{"action":"run_command","value":"/function pve:main/start_check"}}]
execute if score Editmode State matches 1 unless score Game State matches 1 if score InLobby InLobby < Ready InLobby run tellraw @a[tag=Operator] [{"text":"More than 60% of the players in the lobby are ready. \nOnce you're done with editing the map use "},{"text":"/function pve:main/start_check","hoverEvent":{"action":"show_text","contents":[{"text":"Copy to Chat","italic":true}]},"clickEvent":{"action":"suggest_command","value":"/function pve:main/start_check"}}]
